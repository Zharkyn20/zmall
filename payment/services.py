...
class PayboxRedirectService:
    ...
    @classmethod
    def generate_paybox_url(cls, ads_subsriber):
        """generate url for redirect to paybox for payment site rent"""

        paybox_params = {
            "pg_merchant_id": settings.PAYBOX_PROJECT_ID,
            "pg_amount": str(ads_subsriber.pg_amount),
            "pg_currency": settings.PAYBOX_CURRENCY,
            "pg_order_id": str(ads_subsriber.id),
            "pg_salt": settings.PAYBOX_SALT,
            "pg_description": str(ads_subsriber.advertisement.email),
            "pg_language": settings.PAYBOX_LANGUAGE,
            # "pg_success_url": f"{settings.PAYBOX_SUCCESS_URL}{reverse('success_payment')}",
        ...
        return {"url": f"{settings.PAYBOX_URL}?{url_params}",
                'pg_merchant_id': paybox_params.get('pg_merchant_id'),
                'pg_payment_id': paybox_params.get('pg_payment_id'),
                'pg_sig': paybox_params.get('pg_sig'),
                }
